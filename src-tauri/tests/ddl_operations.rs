...
async fn execute_ddl_add_and_drop_column_round_trips() {
    ...
    raw_exec(&driver, "create table ddl_col_test (id serial primary key);").await;
...
async fn execute_ddl_batch_rolls_back_on_failure() {
    ...
    raw_exec(&driver, "create table ddl_rollback_test (id serial primary key);").await;
    ...
    assert!(batch.rolled_back, "batch with a failing statement should be rolled back");
...
async fn execute_ddl_alter_column_renames_and_changes_type() {
    ...
    raw_exec(&driver, "create table ddl_alter_test (id serial primary key, age text);").await;
...
async fn execute_ddl_add_index_and_drop_index() {
    ...
    raw_exec(&driver, "create table ddl_index_test (id serial primary key, email text);").await;
...
async fn execute_ddl_add_check_constraint_and_drop() {
    ...
    raw_exec(&driver, "create table ddl_constraint_test (id serial primary key, price numeric);").await;
    ...
    assert!(constraints.iter().any(|c| c.name == "ddl_constraint_test_price_check"));
