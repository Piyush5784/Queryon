...
async fn list_tables_includes_known_seed_tables() {
    ...
    let tables = schema_service::list_tables(&pool)
        .await
...
async fn get_table_columns_identifies_primary_key() {
    let pool = dev_pool().await;
    let columns = schema_service::get_table_columns(&pool, "public", "users")
        .await
...
async fn get_table_columns_unknown_table_returns_empty() {
    let pool = dev_pool().await;
    let columns = schema_service::get_table_columns(&pool, "public", "table_that_does_not_exist")
        .await
...
async fn fetch_table_rows_respects_limit() {
    let pool = dev_pool().await;
    let result = table_service::fetch_table_rows(&pool, "public", "users", 2, 0)
        .await
...
async fn fetch_table_rows_paginates_without_overlap() {
    ...
    let page1 = table_service::fetch_table_rows(&pool, "public", "users", 2, 0)
        .await
        .expect("page 1 fetch failed");
    let page2 = table_service::fetch_table_rows(&pool, "public", "users", 2, 2)
        .await
...
async fn fetch_table_rows_clamps_oversized_limit() {
    ...
    let result = table_service::fetch_table_rows(&pool, "public", "users", 100_000, 0)
        .await
        .expect("fetch_table_rows failed");
...
async fn fetch_table_rows_rejects_invalid_identifier() {
    let pool = dev_pool().await;
    let result = table_service::fetch_table_rows(&pool, "public", "users; drop table users;--", 10, 0).await;
...
async fn update_json_cell_writes_and_is_readable_back() {
    ...
    let result = table_service::fetch_table_rows(&pool, "public", "users", 10, 0)
        .await
    ...
    table_service::update_json_cell(&pool, "public", "users", &row, "metadata", &json!({ "plan": "free" }))
        .await
...
async fn update_json_cell_fails_when_row_is_missing_primary_key_value() {
    ...
    let result =
        table_service::update_json_cell(&pool, "public", "users", &row, "metadata", &json!({})).await;
...
async fn update_cell_text_writes_a_text_column() {
    ...
    table_service::update_cell_text(&pool, "public", "users", &row, "full_name", Some("Bob Updated"))
        .await
...
async fn update_cell_text_writes_a_boolean_column() {
    ...
    table_service::update_cell_text(&pool, "public", "users", &row, "is_active", Some("false"))
        .await
        .expect("update_cell_text failed");
...
async fn update_cell_text_sets_null() {
    ...
    table_service::update_cell_text(&pool, "public", "orders", &row, "notes", None)
        .await
        .expect("update_cell_text with null failed");

    let result = table_service::fetch_table_rows(&pool, "public", "orders", 10, 0)
        .await
        .expect("fetch after update failed");
...
async fn update_cell_text_rejects_invalid_value_for_column_type() {
    ...
    let result =
        table_service::update_cell_text(&pool, "public", "products", &row, "price_cents", Some("not-a-number"))
            .await;
...
async fn update_cell_text_rejects_unknown_column() {
    ...
    let result =
        table_service::update_cell_text(&pool, "public", "users", &row, "not_a_real_column", Some("x")).await;
...
async fn delete_rows_removes_a_single_row() {
    ...
    let affected = table_service::delete_rows(&pool, "public", "categories", &[row])
        .await
        .expect("delete_rows failed");
    assert_eq!(affected, 1);

    let result = table_service::fetch_table_rows(&pool, "public", "categories", 500, 0)
        .await
        .unwrap();
    let id_index = result.columns.iter().position(|c| c == "id").unwrap();
    assert!(
...
async fn delete_rows_removes_multiple_rows_at_once() {
    ...
    let affected = table_service::delete_rows(&pool, "public", "categories", &[row1, row2])
        .await
        .expect("delete_rows failed");
...
async fn delete_rows_with_empty_input_is_a_noop() {
    let pool = dev_pool().await;
    let affected = table_service::delete_rows(&pool, "public", "categories", &[])
        .await
...
async fn delete_rows_fails_for_nonexistent_row() {
    ...
    let result = table_service::delete_rows(&pool, "public", "categories", &[row]).await;
...
async fn update_cell_text_gives_clean_message_for_foreign_key_violation() {
    ...
    let result =
        table_service::update_cell_text(&pool, "public", "orders", &row, "user_id", Some("999999999")).await;
...
async fn update_cell_text_gives_clean_message_for_not_null_violation() {
    ...
    let result = table_service::update_cell_text(&pool, "public", "orders", &row, "user_id", None).await;
