-- Sample schema for local development of neondb-client against
-- CockroachDB. Mirrors docker/initdb/01_schema.sql (Postgres) — verified
-- against a live CockroachDB container that every construct here
-- (generated always as identity, jsonb, timestamptz, check constraints,
-- foreign keys) works unmodified, since CockroachDB speaks the Postgres
-- wire protocol and its catalog is close enough that PostgresDriver
-- handles it directly (see Engine::CockroachDb's doc comment).
