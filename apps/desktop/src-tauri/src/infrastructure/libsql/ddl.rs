...
use super::metadata::{get_table_columns, list_indexes, quote_ident};
...
async fn render_drop_column(conn: &Connection, table: &str, column: &str) -> Result<String, AppError> {
    let columns = get_table_columns(conn, table).await?;
    let indexes = list_indexes(conn, table).await?;
...
async fn render_alter_column(conn: &Connection, table: &str, edit: &ColumnEdit) -> Result<String, AppError> {
    let columns = get_table_columns(conn, table).await?;
...
async fn render_rebuild(
    ...
    let old_columns = get_table_columns(conn, table).await?;
    let indexes = list_indexes(conn, table).await?;
...
async fn render_add_constraint(conn: &Connection, table: &str, constraint: &NewConstraint) -> Result<String, AppError> {
    let old_columns = get_table_columns(conn, table).await?;
    let indexes = list_indexes(conn, table).await?;
    let temp_table = format!("{table}_bks_rebuild");
...
async fn render_drop_constraint(conn: &Connection, table: &str, constraint: &str) -> Result<String, AppError> {
    let mut rows = conn
        .query(
    ...
    let ddl: String = match rows.next().await.map_err(describe)? {
        Some(row) => row.get(0).map_err(describe)?,
    ...
    if !lower.contains(&needle_quoted) && !lower.contains(&needle_bare) {
        return Err(AppError::new(format!(
            "Constraint '{constraint}' was not declared with a table-level CONSTRAINT clause — LibSQL has no way to drop an inline column constraint (e.g. a column-level UNIQUE) by name. Recreate the table instead."
        )));
    }
...
async fn render_one(conn: &Connection, statement: &DdlStatement) -> Result<String, AppError> {
    match statement {
...
pub async fn render_all(conn: &Connection, statements: &[DdlStatement]) -> Result<Vec<DdlPreview>, AppError> {
    let mut results = Vec::with_capacity(statements.len());
    for statement in statements {
        let sql = render_one(conn, statement).await?;
        results.push(DdlPreview { sql });
    }
    Ok(results)
}

pub async fn execute_all(conn: &Connection, statements: &[DdlStatement]) -> Result<DdlBatchResult, AppError> {
    let mut results = Vec::with_capacity(statements.len());

    for statement in statements {
        let sql = match render_one(conn, statement).await {
            Ok(sql) => sql,
            Err(e) => {
        ...
        match conn.execute_batch(&sql).await {
            ...
            Err(e) => {
                results.push(DdlExecutionResult { sql, success: false, error: Some(crate::error::describe_libsql_error(&e)) });
                break;
            }
        }
    }
